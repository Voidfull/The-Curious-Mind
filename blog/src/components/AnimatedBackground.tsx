...
/* ========================================
   ...
   Flowing particles that travel along curves,
   leave faint trails, and connect nearby particles
   ======================================== */
function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    const colors = {
      dark: [
        [232, 132, 90],   // accent orange
        [144, 112, 192],  // lavender
        [74, 136, 168],   // ocean
        [107, 153, 112],  // sage
        [184, 152, 64],   // gold
        [192, 96, 120],   // rose
    ...
    const draw = () => {
      ...
      const baseAlphaMult = dark ? 1.2 : 0.7;

      // Update & draw particles
      for (const p of particles) {
        // Wander — organic curved motion
        ...
        const pulseAlpha = p.alpha * (0.6 + Math.sin(p.pulse) * 0.4) * baseAlphaMult;
        ...
        if (p.trail.length > 2) {
          ...
          grad.addColorStop(1, `rgba(${p.color},${pulseAlpha * 0.3})`);
        ...
        if (dark && p.radius > 1.5) {
          ...
          glow.addColorStop(0, `rgba(${p.color},${pulseAlpha * 0.25})`);
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < connectionDist) {
            const alpha = (1 - dist / connectionDist) * 0.08 * baseAlphaMult;
            ctx.beginPath();
...
/* ========================================
   CSS-based decorative layers on top of canvas
   ======================================== */

function AuroraWave({ color, delay, duration, y }: { color: string; delay: number; duration: number; y: string }) {
  return (
    <div
...
function InkBlot({ path, x, y, size, color, rotateDuration, delay }: {
  path: string; x: string; y: string; size: number; color: string;
  rotateDuration: number; delay: number;
}) {
  return (
    <svg
...
function SacredRing({ x, y, size, color, duration, reverse }: {
  x: string; y: string; size: number; color: string; duration: number; reverse?: boolean;
}) {
  return (
    <svg
      className="absolute pointer-events-none"
...
function FloatingGlyph({ char, x, y, size, color, delay, duration }: {
  char: string; x: string; y: string; size: string; color: string; delay: number; duration: number;
}) {
  return (
    <div
      className="absolute font-serif select-none pointer-events-none italic"
      style={{
        left: x, top: y, fontSize: size, color,
...
export default function AnimatedBackground() {
  ...
  const blotPaths = [
    "M44.7,-76.4C58.8,-69.2,71.8,-59.1,79.6,-45.8C87.4,-32.5,90,-16.3,88.5,-0.9C87,14.5,81.4,29,72.3,40.4C63.2,51.8,50.6,60.2,37.2,67.2C23.8,74.3,9.5,80.1,-4.8,87.2C-19.1,94.4,-33.4,103,-45.5,98.5C-57.5,93.9,-67.4,76.3,-74.3,59.4C-81.2,42.5,-85.1,26.3,-85.8,10.2C-86.5,-5.9,-83.9,-21.8,-76.2,-34.5C-68.4,-47.1,-55.5,-56.4,-42.1,-64C-28.7,-71.7,-14.3,-77.7,0.7,-78.8C15.8,-80,30.5,-83.5,44.7,-76.4Z",
    ...
    "M47.2,-73.7C61.8,-68.6,74.7,-57.2,81.7,-43.2C88.7,-29.1,89.9,-12.4,87.6,3.4C85.3,19.2,79.5,34.1,70.8,46.8C62.1,59.5,50.5,70,37.3,76.6C24.1,83.2,9.3,85.9,-5.2,87.6C-19.7,89.3,-33.9,90,-45.6,83.8C-57.3,77.6,-66.5,64.5,-73.8,50.3C-81.1,36.1,-86.5,20.8,-85.9,5.7C-85.3,-9.4,-78.7,-24.3,-69.6,-37.1C-60.5,-49.9,-48.9,-60.6,-35.9,-66.4C-22.9,-72.2,-8.5,-73.1,6.1,-79.4C20.7,-85.7,32.6,-78.8,47.2,-73.7Z",
  ];

  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
      {/* Layer 1: Canvas particle system — flowing dots with trails and connections */}
      <ParticleCanvas />

      {/* Layer 2: Aurora / Northern Lights waves */}
      <AuroraWave color={`${accent}0.04)`} delay={0} duration={30} y="5%" />
      <AuroraWave color={`${lavender}0.03)`} delay={8} duration={40} y="35%" />
      <AuroraWave color={`${ocean}0.03)`} delay={15} duration={35} y="65%" />

      {/* Layer 3: Pulsing halos — ripple outward and fade */}
      <PulsingHalo x="20%" y="25%" size={200} color={`${accent}0.2)`} delay={0} />
      <PulsingHalo x="20%" y="25%" size={200} color={`${accent}0.15)`} delay={2} />
      <PulsingHalo x="20%" y="25%" size={200} color={`${accent}0.1)`} delay={4} />
      <PulsingHalo x="80%" y="70%" size={180} color={`${lavender}0.18)`} delay={1} />
      <PulsingHalo x="80%" y="70%" size={180} color={`${lavender}0.12)`} delay={3} />
      <PulsingHalo x="80%" y="70%" size={180} color={`${lavender}0.08)`} delay={5} />
      <PulsingHalo x="50%" y="50%" size={300} color={`${sage}0.1)`} delay={0.5} />
      <PulsingHalo x="50%" y="50%" size={300} color={`${sage}0.07)`} delay={2.5} />

      {/* Layer 4: Organic ink blots */}
      <InkBlot path={blotPaths[0]} x="-3%" y="8%" size={280} color={`${accent}0.06)`} rotateDuration={80} delay={0} />
      <InkBlot path={blotPaths[1]} x="72%" y="3%" size={240} color={`${lavender}0.05)`} rotateDuration={65} delay={5} />
      <InkBlot path={blotPaths[2]} x="55%" y="55%" size={220} color={`${ocean}0.05)`} rotateDuration={70} delay={10} />
      <InkBlot path={blotPaths[0]} x="10%" y="70%" size={200} color={`${sage}0.04)`} rotateDuration={90} delay={15} />

      {/* Layer 5: Sacred geometry rings */}
      ...
      <SacredRing x="70%" y="72%" size={220} color={strokeColor} duration={55} />
      <SacredRing x="25%" y="78%" size={130} color={strokeColor} duration={32} reverse />

      {/* Layer 6: Floating typography — more visible */}
      <FloatingGlyph char="&" x="10%" y="12%" size="10rem" color={`${accent}${glyphOpacity})`} delay={0} duration={16} />
      <FloatingGlyph char="∞" x="86%" y="18%" size="7rem" color={`${lavender}${glyphOpacity})`} delay={4} duration={20} />
      <FloatingGlyph char="§" x="6%" y="68%" size="8rem" color={`${ocean}${glyphOpacity})`} delay={8} duration={18} />
      <FloatingGlyph char="✦" x="78%" y="82%" size="6rem" color={`${accent}${glyphOpacity})`} delay={12} duration={22} />
      <FloatingGlyph char="¶" x="90%" y="55%" size="5rem" color={`${sage}${glyphOpacity})`} delay={6} duration={14} />

      {/* Layer 7: Grid (dark mode only) */}
      {isDark && (
